...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  const menuGroups = [
    {
      menuItems: [
        ...
        {
          icon: <RiUserLine  size={23}/>,

          label: "Profile",
          route: `/profile/${params.id}`,
        },
        {
          icon: <FiPercent    size={23}/>,

          label: "Comissions Rate",
          route: `/comissionrate/${params.id}`,
        },
  ...
  const menuGroups1 = [
    {
      menuItems: [
        // {
        //   icon: 
        //     <TbMessageReply size={26} />,

          
        //   label: "Al Comment Reply",
        //   route: `/aireplier/${params.id}`,
        // },
        // {
        //   icon: (
        //     <LuUsers2 size={24} />
        ...
        //   ),
        //   label: "Al Deal closer",
        //   route: `/Aidealcloser/${params.id}`,
        // },
  ...
  return (
    <ClickOutside onClick={() => setSidebarOpen(false)}>
      <aside
        className={`absolute left-0 top-0 z-9999 flex h-screen w-52.5 flex-col overflow-y-hidden border-r border-stroke bg-white dark:border-stroke-dark dark:bg-gray-dark lg:static lg:translate-x-0 ${sidebarOpen
        ...
        <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
          ...
          <nav className="mt-1 px-4 lg:px-1">
            {menuGroups.map((group, groupIndex) => (
              <div key={groupIndex}>
                <h3 className="mb-5 text-lg font-medium text-dark-4 dark:text-dark-6 px-2">
                </h3>
